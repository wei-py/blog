---
date: 2024-01-20
title: VSCode 配置指南 - 打造高效开发环境
category: 编辑器
tags:
  - vscode
  - 配置
  - 开发工具
description: 详细记录 VSCode 的个性化配置,包括主题、字体、快捷键、插件等设置,打造符合个人习惯的高效开发环境
---
...
- 使用 FiraCode Nerd Font Mono 作为主要字体,支持连字符
- 中文回退字体使用霞鹜文楷 GB
- 启用字体连字,提升代码可读性
...
当前使用 Dioptase (Light) 亮色主题,也配置了多个暗色主题备用:
...
"package.json": "_.lock, .npmrc, .nvmrc, tsconfig._, _.config._",
"_.ts": "$(capture).js, $(capture).d.ts.map, $(capture)._.ts",
"_.tsx": "$(capture).ts, $(capture)._.tsx, $(capture).css"
}
}
...
1. **Vim 模式**: 完整的 Vim 快捷键支持,提升编辑效率
2. **美观舒适**: 精心调整的字体和主题配色
3. **智能提示**: 配置了 AI 辅助工具
4. **文件管理**: 合理的文件嵌套和排序规则
5. **多语言支持**: 针对不同语言的格式化配置

通过这些配置,VSCode 成为了一个高效、舒适的开发环境。
